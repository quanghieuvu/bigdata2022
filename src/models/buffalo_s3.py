...
class Model():
	...
	def run_epoch(self, loader):
		if loader.is_training:
			...
			# if self.epoch_id <= self.lr_step: 
			# 	self.model_image.train()
			# else:
			# 	self.model_image.eval()
			# 	for child in self.model_image.children():
	...
	def run_batch(self, loader, batch_data):
		...
		# pred_encode, feature_encode = None, encode 
		...
		# print ('image_decryption_loss = {:.6f}'.format(self.image_decryption_loss.item()))
		if pred_encode is not None:
			# print ('label', label.size())
			# print ('encode', encode.size(), torch.min(encode), torch.max(encode))
			# print ('pred_encode', pred_encode.size(), torch.min(pred_encode), torch.max(pred_encode))
			self.encode_decryption_loss = self.criterion(pred_encode, encode)
			# print ('encode_decryption_loss', self.encode_decryption_loss.item())
		...
		self.loss = self.image_decryption_loss + self.encode_decryption_loss + triplet_loss / 10
		# self.loss = self.image_decryption_loss + loss_similarity + loss_difference
		# if self.epoch_id <= self.lr_step:
