...
class Arch(nn.Module):
    def __init__(self, output_depth=3, skip_connection=True):
        ...
        self.rnn_0 = nn.RNN(input_size=self.INPUT_LSTM_0, hidden_size =self.HIDDEN_SIZE_0, num_layers =self.NUM_LAYERS_0, 
    ...
    def forward(self, binary_encode):
        ...
        rnn_0, _ = self.rnn_0(binary_encode, h0)
        ...
        # global_feature = lstm_0[:, -1, :]
        global_feature = torch.mean(rnn_0, axis=1)
        # global_feature = torch.unsqueeze(global_feature, 2)
        # global_feature = torch.unsqueeze(global_feature, 3)
        # feature_map5 = self.mid_init(global_feature)
