...
# 3x3 convolution
def conv63(in_channels, out_channels, stride=1):
    return [nn.Conv1d(in_channels, out_channels, kernel_size=63, 
                     stride=stride, padding=21, bias=False),
            nn.BatchNorm1d(out_channels),
            nn.ReLU(inplace=True)]

def conv31(in_channels, out_channels, stride=1):
    return [nn.Conv1d(in_channels, out_channels, kernel_size=31, 
                     stride=stride, padding=15, bias=False),
            nn.BatchNorm1d(out_channels),
            nn.ReLU(inplace=True)]

def conv15(in_channels, out_channels, stride=1):
...
class Arch(nn.Module):
    def __init__(self, output_depth=3, skip_connection=True):
        ...
        self.conv_4 = nn.Sequential(*(conv63(1, 8) + conv1(8, 8) + conv63(8, 8, 32)))
        N = 8
        self.conv_8 = nn.Sequential(*(conv63(N, N) + conv1(N, N) + conv63(N, N*2, 4)))
        N *= 2
        self.conv_16 = nn.Sequential(*(conv63(N, N) + conv1(N, N) + conv63(N, N*2, 4)))
        N *= 2
        self.conv_32 = nn.Sequential(*(conv63(N, N) + conv1(N, N) + conv63(N, N*2, 4)))
        N *= 2
        self.conv_64 = nn.Sequential(*(conv31(N, N) + conv1(N, N) + conv31(N, N*2, 2)))
        N *= 2
        self.conv_128 = nn.Sequential(*(conv31(N, N) + conv1(N, N) + conv31(N, N*2, 2)))
        N *= 2
        self.conv_256 = nn.Sequential(*(conv31(N, N) + conv1(N, N) + conv31(N, N*2, 2)))
        N *= 2
        self.conv_512 = nn.Sequential(*(conv31(N, N) + conv1(N, N) + conv31(N, N*2, 2)))
        N *= 2
        self.conv_1024 = nn.Sequential(*(conv15(N, N) + conv1(N, N)))
